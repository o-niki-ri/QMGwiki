...
12. Develop for 10 seconds then immediately into water.

13. Dry surface with N₂, bottom side with Kimtech.
...
2. Vent vacuum → Open Lid → Load sample → Close lid → Turn on vacuum and wait
...
4. Turn gas on → When gas is ready, turn RF on for 10 seconds.
...
6. Make sure pressure drops to 0.5

   → SEKI is powered by the bottom right switch (under the big CVD) and is marked with AXSID. Once you hear air hissing it means it is on.

7. Turn on computer (pw: PAT2), open IE and select appropriate gas for your application (H₂, N₂ and ?)
   - Check flow point: 0
...
6. Bring down stage very slowly by dropping screws 1 mm each, vent a little Ar₂ (green screw)
...
![PDMS](../_static/pdms.png){ width="200" }

**Option 1:** (if only a little is needed)

- Mix on glass slide or small plastic weigh boat
...
- EtOH (for cleaning)
- A clean tube
- Vacuum components (clamps, allan keys, o-rings → in the drawer beneath the furnace)
...
**Program (setting annealing temp):** Use `P`, `^`, `˅`, `<` buttons

For top menu, press `P`:

| Setting | Description |
|---------|-------------|
| Pr n | Select the program number |

Sub menu, press `P` + `<` together x 2
...
!!! tip
    Thickness is especially important. Thicker flakes generally require a lower electron dose to activate emitters.
...
The Helios SEM can pattern arrays in multiple ways. Either individual spots (set the circle pattern to 0um radius) using the array builder. This works well if a dose series is required as each spot can have a specific dose. If larger arrays of the same dose are required, the rectangular pattern tool can be used. Individual spots can be patterned by increasing the pixel spacing to achieve the desired distance between spots. By default it will be very small eg. <100 nm.
...
- **Carbon contamination:** Aim to have the cleanest hBN possible before beginning irradiations. Utilise one or more of hotplate annealing, plasma cleaning, ozone cleaning where applicable. Utilise ozone cleaning after irradiation to reduce the background PL from carbon deposition.

- **Drift:** Drift can be an issue when patterning with the SEM. If this occurs attempt to change the beam energy to reduce this, or use immersion mode.

**More Info:** [Gale et.al. (2022)](https://pubs.acs.org/doi/10.1021/acsphotonics.2c00631)
...
**More info:** [Hennessey et.al. (2024)](https://doi.org/10.1002/qute.202300459) (Comparing energies/beam species/carbon contamination/sputter yields)

!!! note
    Hydrogen does create VB⁻ but nitrogen is more reliable.
